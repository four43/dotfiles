---
name: confluence
description: Knowledge base page management via Confluence and the `confluence` CLI. Use when the user wants to browse, search, create, or edit Confluence knowledge base pages.
---
...
The `create` and `edit` commands use a markdown-ish format in the editor:

```markdown
# Heading 1              ## Heading 2
**bold** *italic* `code` - list item
> blockquote             --- (horizontal rule)
```

Panels use GitHub-style admonitions:
...
Supported: `[!INFO]`, `[!NOTE]`, `[!WARNING]`, `[!TIP]`.

Other special syntax:

- Code blocks: ` ```python ... ``` `
- Status lozenges: `{status:Done|Green}`
- Confluence macros (excerpt, drawio, jira, etc.) appear as raw XML inside
  `~~~confluence-macro` fences — these are preserved exactly and can be
  edited carefully.

To **add an excerpt** to a new page, include the raw macro XML:

```xml
~~~confluence-macro
<ac:structured-macro ac:name="excerpt" ac:schema-version="1">
<ac:rich-text-body><p>Your excerpt text here.</p></ac:rich-text-body>
</ac:structured-macro>
~~~
```
